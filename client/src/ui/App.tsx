...
const App: React.FC = () => {
  ...
  // ---- Load knowledge.json at runtime (no tokens) ----
  useEffect(() => {
  ...
  const normalizeEventKey = (label?: string): string => {
    if (!label) return "";
    let s = label.trim();
    // Emoji + Leerzeichen am Anfang grob entfernen
    s = s.replace(/^([\u{1F300}-\u{1FAFF}]\uFE0F?|\p{Extended_Pictographic})\s*/gu, "");
    // (z. B. …)-Anhang entfernen
    s = s.replace(/\s*\(z\.\s*B\..*?\)\s*$/i, "");
    // Normalisieren auf unsere Keys
  ...
  // ---- App start greeting (after K is ready) ----
  useEffect(() => {
    if (!K) return;
    setMessages([
      {
  ...
  const onChoice = (choice: string) => {
    ...
    // Step 3: Gästezahl (inkl. special_contexts)
    if (currentStepId === 3) {
  ...
  // Beim Eintritt in Step 5 den ersten Substep ankündigen
  useEffect(() => {
    if (!K) return;
    if (currentStepId === 5) {
...
function buildPriceText(sel: Selections, K: Knowledge) {
  ...
  if (chosen.length > 1) {
    const extras = chosen.length - 1; // ein Paket inkl.
    items.push({
...
// Fallback-Normalizer ohne fancy Unicode
