...
type Knowledge = { brand?: string; assistant_name?: string; theme?: { bg?: string; text?: string; brand?: string; accent?: string }; disclaimer?: string; };
...
function useKnowledge() {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      try { const res = await fetch("/knowledge.json"); const data = res.ok ? ((await res.json()) as Knowledge) : null;
        if (!alive) return; setK(data ?? { brand: "Fobi Fotobox", assistant_name: "Dennis", theme: { bg:"#1e1e1c", text:"#ffffff", brand:"#ccb63f", accent:"#ccb63f" }});
      } catch { if (!alive) return; setK({ brand: "Fobi Fotobox", assistant_name: "Dennis", theme: { bg:"#1e1e1c", text:"#ffffff", brand:"#ccb63f", accent:"#ccb63f" }}); }
    })(); return () => { alive = false; };
  }, []);
  return k;
}
      ...
      <main className="chat">
        ...
        <div className="bubble sum">
          ...
          {appSel.mode === "Digital & Print" && (<>
            ...
            {appSel.printpkg ? (appSel.format === "Streifen" ? (
              <div className="hint" style={{ marginTop: 4 }}>{parseInt(appSel.printpkg||"0")} Fotostreifen (Printpaket {Math.round(parseInt(appSel.printpkg||"0")/2)})</div>
            ) : (
              <div className="hint" style={{ marginTop: 4 }}>{parseInt(appSel.printpkg||"0")} Postkarten</div>
            )) : null}
          </>)}
          ...
          {(() => {
            ...
            return appSel.accessories?.length ? appSel.accessories.map(a => {
              const isIncluded = a === includedSmall; const price = isIncluded ? 0 : (ACCESSORY_PRICE_MAP[a] ?? 0);
              return (<div key={a} className="sumrow"><span>{a}</span><b>{isIncluded ? "inkl." : price.toFixed(2) + " €"}</b></div>);
            }) : null;
          })()}
          <div className="sumrow" style={{ marginTop: 8 }}><span><b>Gesamt</b></span><b>{computePrice(appSel).toFixed(2)} €</b></div>
        </div>
      </main>
    </div>
  );
